...
public class PlayerController implements Initializable {
    ...
    private void handleReady() {
        ...
        lengthText.setText(Double.toString(Math.round(d*10.0)/10.0));
        Duration ct = mediaPlayer.getCurrentTime();
        double c = ct.toMillis();
        currentText.setText(Double.toString(Math.round(c*10.0)/10.0));
        currentVisualizer.start(numBands, vizPane);
        timeSlider.setMin(0);
    ...
    @FXML
    private void handleSlideStart(Event event){
        if(mediaPlayer != null){
            mediaPlayer.pause();
        }
    }
    ...
    @FXML
    private void handleSlideStop(Event event){
        if(mediaPlayer != null){
    ...
    private void handleUpdate(double timestamp, double duration, float[] magnitudes, float[] phases) {
        ...
        currentText.setText(Double.toString(Math.round(ms*10.0)/10.0));
